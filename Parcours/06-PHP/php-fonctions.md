...
- [Fonctions](php-fonctions.md) :point_left:
...
Cela fonctionne, mais est-ce que c'est DRY ? Non. On répète 4 fois le même code. Le jour où l'on devra modifier ce programme pour une raison ou l'autre, il faudra le modifier à 4 endroits, augmentant d'autant de fois le risque d'introduire un bug. :poop:
...
La fonction `str_shuffle()` (lire : ["**shuffle** **str**ing"](http://php.net/manual/fr/function.str-shuffle.php)) permet de mélanger les caractères d'une chaîne (autrement dit : un bout de texte).
...
Une répétition ! Ton subsconscient aussitôt crie *DRY! DRY! DRY!*  
